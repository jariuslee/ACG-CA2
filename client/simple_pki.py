"""
client/simple_pki.py
---------------------

Purpose
    Verification-only PKI helper used by the client. It loads the server's
    CA certificate (PEM) and verifies user certificates that the server
    issued. The client never creates or stores a CA private key.

Key ideas
    - CA lives on the server. The server creates/loads its RSA-2048 CA and
      signs user certificates that bind: username ↔ (Ed25519_pub, X25519_pub).
    - Client fetches the CA certificate (PEM) and uses it to verify user
      certificates before trusting any public keys. This prevents MITM
      key-swaps.
    - Certificates embed user data inside a custom X.509 extension with OID
      1.2.3.4.5.6.7.8.9 as a JSON blob containing:
          { "username", "ed25519_public_key", "x25519_public_key", ... }

PEM quick note
    PEM is a Base64-encoded text format with BEGIN/END headers. We load CA and
    user certificates from PEM strings.

Scope & safety
    - This module is used in the client only in verification_only=True mode.
    - Issuance methods exist for compatibility but must NOT be used on client.
      The client must never hold a CA private key.
"""
...
class SimpleCertificateAuthority:
    """
    CLIENT SIDE PKI that can verify server-issued certificates
    """
    
    def __init__(self, ca_name: str = "SecureMessaging CA", verification_only: bool = False): # states that its only for verification
